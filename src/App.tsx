...
import { getTopPerformingClasses, getClassDuration, calculateTeacherHours, getClassCounts, validateTeacherHours, getTeacherSpecialties, getClassAverageForSlot, getBestTeacherForClass, generateIntelligentSchedule } from './utils/classUtils';
...
function App() {
  ...
  const handleAutoPopulateTopClasses = async (data: ClassData[] = csvData) => {
    ...
    try {
      ...
      if (topClasses.length === 0) {
        alert('No top performing classes found in the data');
        setIsPopulatingTopClasses(false);
        return;
      }

      // Create scheduled classes from top performers
      const newScheduledClasses: ScheduledClass[] = [];
      const teacherHoursTracker: Record<string, number> = {};

      // Limit to prevent overwhelming the schedule
      const classesToSchedule = topClasses.slice(0, 30);

      for (const topClass of classesToSchedule) {
      ...
      console.log('Scheduled classes created:', newScheduledClasses.length);
  ...
  const handleAutoOptimize = async () => {
    ...
    try {
      // Generate intelligent schedule using local optimization
      ...
      console.log('Generated optimized schedule:', optimizedSchedule.length, 'classes');
      ...
      alert(`Generated optimized schedule with ${optimizedSchedule.length} classes using intelligent algorithms!`);
    } catch (error) {
